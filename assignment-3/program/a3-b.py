...
def render(painter, colors, sizes, problem, search, solution):
    ...
    for coordinate, symbol in symbols.items():
        vi.qt.grid.draw_cell_text(
            painter, problem.grid, sizes['cell'], coordinate, symbol)
...
problem  = load_fancy_board_file(input_file_name)
search   = vi.search.graph.BestFirst(
    problem, vi.search.graph.BestFirst.Strategy[sys.argv[1]])
...
colors = {
    ...
    'cells': {
        100: QColor(77, 77, 255),
        50:  QColor(166, 166, 166),
        10:  QColor(0, 128, 0),
        5:   QColor(128, 255, 128),
        1:   QColor(191, 128, 64)
    }
}
...
render(painter, colors, sizes, problem, search, solution)
painter.end()
